...
import {
  saveIntakeBasics, saveIntakeSport, saveIntakeTeam, resetIntake,
...
// Starting step is derived from what's already saved on the athlete — so a
// signed-in user with a full profile lands on their home, a partially-filled
// intake resumes on the right step, and a brand-new account starts at intro.
function startingStep(i: IntakeInitial): Step {
  if (!i.hasSignedIn) return "intro";
  const hasBasics = !!i.basics.firstName && !!i.basics.dob;
  if (!hasBasics) return "basics";
  if (!i.sport) return "sport";
  const isTier1 = TIER1_SPORTS.some((s) => s.label === i.sport);
  if (isTier1 && !i.team) return "team";
  return "home";
}

export default function IntakeFlow({ initial }: { initial: IntakeInitial }) {
  const router = useRouter();
  const [step, setStep] = useState<Step>(() => startingStep(initial));
  const [basics, setBasics] = useState(initial.basics);
  const [sport, setSport] = useState<string | null>(initial.sport);
  const [sportId, setSportId] = useState<Sport | null>(initial.sportId);
  const [team, setTeam] = useState<Team | null>(initial.team);
  const [, startTransition] = useTransition();

  const clearDraft = () => {
    startTransition(async () => {
      await resetIntake();
      setBasics({ firstName: "", lastName: "", dob: "", state: "" });
      setSport(null);
      setSportId(null);
      setTeam(null);
      setStep("intro");
        ...
        {step === "basics" && (
          <Fade key="basics">
            <BasicsStep
              ...
              onNext={() => {
                startTransition(async () => {
                  await saveIntakeBasics({
                    firstName: basics.firstName,
                    lastName: basics.lastName,
                    dob: basics.dob,
                    homeState: basics.state,
                  });
                  setStep("sport");
                });
              }}
              ...
              onConfirm={() => {
                ...
                startTransition(async () => {
                  await saveIntakeTeam({
                    ...
                    manualNote: team.id === "manual" ? team.school : null,
                  });
                  setStep("confirm");
                });
              }}
    ...
    <section className="py-16 md:py-24">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="mt-16 border-t border-line pt-8 flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            onClick={() => {
              if (confirm("Reset your intake? Your goals, contracts, and expenses stay — this only clears name/sport/team.")) {
                onStartOver();
              }
            }}
            className="text-xs text-slate hover:text-accent underline underline-offset-4"
          >
            Start intake over
          </button>
        </div>
